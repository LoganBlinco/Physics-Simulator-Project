...
public class GraphMaker : MonoBehaviour
{
    ...
    public static bool graphCreated = false;
    ...
    public void AddPoint(Vector2 point)
    {
        if ((point.x > minX && point.x < maxX) && (point.y > minY && point.y < maxY))
        {
            ...
            Vector3 position = new Vector3(
                (transform.position.x + HorizontalHeight * leftPaddingPercent) + (point.x - minX) * xRatio,
                (transform.position.y + VerticalHeight * bottomPaddingPercent) + (point.y - minY) * yRatio,
                0.0f);
            ...
            Vector3 delta = -(pointsObjects[pointsSize-1].transform.position - pointsObjects[pointsSize].transform.position);

            //Calculates angle between first and second point
            float angle = Mathf.Atan2(delta.y, delta.x) * Mathf.Rad2Deg;
            //Position to begin the line
            Vector3 Position = pointsObjects[pointsSize-1].transform.position;
